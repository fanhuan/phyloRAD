...
#Stats after random dropout:
hist = collections.Counter(total_list)
hist_after_dropout = collections.Counter(total_list_after_dropout)
#total = sorted(set(item for list in sba_selected.values() for item in list))
#sba_list = sorted(reduce(set.intersection,map(set,sba_selected.values())))
