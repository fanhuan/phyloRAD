...
# -*- coding: utf-8 -*-
#
#  phylip_file_generator_consense_SBAonly.py
...
def phylip_writer_SBA(handle_in,handle_out,nloci,loci_list,sba_list,sample,read_len):
	alignment = {}
...
outhandle.write('%d\t%d'%(len(samples),read_len*(len(sba_list)-1)))
#-1 is because there is -1 in the list
...
for sample in samples:
	if type == 'single-end':
		...
		filehandle = open(os.path.join(options.dataDir, sample+'.fa'))
		phylip_writer_SBA(filehandle,outhandle,options.nloci,loci_list,sba_list,sample,read_len)
	else:
